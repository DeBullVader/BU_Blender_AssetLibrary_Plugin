...
def set_camera_look_at_vector(pivot_point,camera):
...
def get_scale_factor(obj,max_scale):
    target_x_size =max_scale.x
    target_y_size =max_scale.y
    target_z_size =max_scale.z
...
def calc_col_scale_factor(collection, min_scale=0.5, max_scale=1.5):
    max_coords, min_coords = get_collection_bounding_box(collection)
    world_bbox_size = max_coords - min_coords

    max_dimension = max(world_bbox_size.x, world_bbox_size.y, world_bbox_size.z)
    min_dimension = min(world_bbox_size.x, world_bbox_size.y, world_bbox_size.z)

    scale_factor = min(max_scale / max_dimension, max(min_scale / min_dimension, 1))
  
    return scale_factor

def get_col_scale_factor(collection, max_scale):
    max_coords,min_coords = get_collection_bounding_box(collection)
    world_bbox_size =max_coords-min_coords
    increase = 1.4
    if world_bbox_size.x<world_bbox_size.y/1.3:
        max_scale.x *= increase
        max_scale.y *= increase
    elif world_bbox_size.y<=world_bbox_size.x/1.3:
        max_scale.x *= increase
        max_scale.y *= increase

    print(world_bbox_size[0])

    scale_factor_x = max_scale.x / world_bbox_size[0] if world_bbox_size[0] != 0 else 1
    scale_factor_y = max_scale.y / world_bbox_size[1] if world_bbox_size[1] != 0 else 1
    scale_factor_z = max_scale.z / world_bbox_size[2] if world_bbox_size[2] != 0 else 1
    scale_factor = min(scale_factor_x,scale_factor_y, scale_factor_z)
    return scale_factor
...
def scale_asset_for_render(scene,object_to_render,max_scale):
    ...
    set_bottom_center(object_to_render)
    scene.view_layers[0].update()
